...
import (
	...
	"github.com/aramase/secrets-store-controller/pkg/k8s/token"
)
...
func (c *TokenClient) SecretProviderServiceAccountTokenAttrs(namespace, serviceAccountName string, tokenRequests []secretsstorecsixk8siov1.TokenRequest) (map[string]string, error) {
	...
	for _, tokenRequest := range tokenRequests {
		...
		tr := &authenticationv1.TokenRequest{
			Spec: authenticationv1.TokenRequestSpec{
				...
				Audiences:         audiences,
				// TODO(aramase): set the bound object ref
	...
	klog.V(4).InfoS("Fetched service account token attrs", "serviceAccountName", serviceAccountName, "namespace", namespace)
